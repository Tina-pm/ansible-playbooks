...
clean_mount_point(){
  ...
  # Remove mount point directory but maybe nothing is mounted due to an interruption
...
get_backup_location(){
  ...
  local device_name=$1
  echo "$BACKUP_FROM_DIR/$(dmsetup info -C --noheadings -o name $device_name | sed 's/--snap$//')"
}
...
is_filesystem(){
  # Exit with 0 if the parameter is a device with a supported filesystem, 1 otherwise.
  local device=$1
  grep -q "\b$(blkid $device | sed 's/.* TYPE="\([^"]*\)".*/\1/')$" /proc/filesystems
}
...
do
  ...
  if is_partition_table /dev/$volume-snap
  then
    dd if=/dev/$volume-snap of="$(get_backup_location /dev/$volume).mbr" bs=512 count=1
